...
export default function AccLayout({
  ...
  useEffect(() => {
    sanityClient.fetch(coursesSlugsQuery).then((data) => {
      setCourses(data);
      console.log(data);
    });
  }, []);
  ...
  return (
    <motion.div
      ...
      className="bg-black overflow-hidden w-screen h-screen account flex items-center justify-center px-6 lg:px-32 2xl:px-44"
    >
      <div className="relative w-full h-[80vh] 3xl:h-[83vh] rounded-3xl border-[#2B2B2B] border-[1px] flex flex-row">
        <aside className="z-[1] w-[20%] h-full border-r-[1px] border-[#2b2b2b] relative flex flex-col justify-between p-6">
          <div className="flex flex-col items-start gap-3">
            ...
            />
            <div className="flex flex-col mt-16 px-2 w-full gap-1">
              <h3 className="text-lg text-[#818181] font-medium mb-3">
                Dashboard
              </h3>
              <LeftItem
                icon={
        ...
        <main className="w-[75%] bg-gradient-to-r from-[#1e1e1e]/20 to-[#818181]/0 z-[1] relative flex flex-col items-start justify-center py-10 pl-[15%] gap-12">
          {children}
        </main>
      </div>
    </motion.div>
