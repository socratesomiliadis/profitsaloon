...
export default function Button({
  ...
  return (
    <Link
      ...
      className={`${
        theme === 'primary'
          ? 'bg-white text-[#202020] px-16'
          : 'bg-transparent text-[#ededed] px-10'
      } ${
        theme === 'secondary' ? 'border-[1px] border-[#818181]' : 'border-[0px]'
      } rounded-full py-4  flex flex-row items-center gap-2`}
    >
      <span className="font-medium text-xl -mb-1">{text}</span>
      {hasArrow && (
        <span
